...
# async def create_order(data: CreateOrderRequest):
#     try:
#         print(f"Creating order with amount: {data.amount}")  # Debug print to verify amount
#         options = {
#             "amount": data.amount,  # ✅ Keep amount as-is; it's already in paise
#             "currency": data.currency,
#             "receipt": data.receipt
#         }
#         order = razorpay_client.order.create(options)
...
# async def verify_order(data: VerifyOrderRequest):
#     try:
#         secret = "a6ReEJTcPIhrQolLWM6xp4sG"  # ✅ Added actual secret key
#         generated_signature = hmac.new(
#             key=bytes(secret, 'utf-8'),
#             msg=bytes(data.razorpay_order_id + "|" + data.razorpay_payment_id, 'utf-8'),
#             digestmod=hashlib.sha256
...
#     except Exception as e:
#         print(e)
#         raise HTTPException(status_code=500, detail="seccessfull")
